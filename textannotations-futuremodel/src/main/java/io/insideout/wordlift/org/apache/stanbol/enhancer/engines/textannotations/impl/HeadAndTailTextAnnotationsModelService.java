...
@Service
public class HeadAndTailTextAnnotationsModelService implements
		TextAnnotationsModelService {

	// a reference to the TextAnnotationService support class that will be used
	// to extract TextAnnotations
	...
	// the number of characters to capture for head/tail and prefix/suffix
	// substrings.
	...
	private final static UriRef FISE_SELECTION_PREFIX_URI = new UriRef(
			NamespaceEnum.fise + "selection-prefix");
	private final static UriRef FISE_SELECTION_HEAD_URI = new UriRef(
			NamespaceEnum.fise + "selection-head");
	private final static UriRef FISE_SELECTION_TAIL_URI = new UriRef(
			NamespaceEnum.fise + "selection-tail");
	private final static UriRef FISE_SELECTION_SUFFIX_URI = new UriRef(
	 ...
	 */
	public Collection<TextAnnotation> enhanceTextAnnotationsForContentItem(
	...
	public Collection<TextAnnotation> enhanceTextAnnotations(String text,
		...
		for (TextAnnotation textAnnotation : textAnnotations) {
			...
			int tailLength = (selectionLength < (textLength - end) ? selectionLength
					: (textLength - end));

			int annotationLength = (selectionLength < annotationText.length() ? selectionLength
					: annotationText.length());

			if (start - headLength > 0 && start < text.length())
				textAnnotation.setSelectionHead(text.substring(start
						- headLength, start));
			else
				textAnnotation.setSelectionHead("");
			textAnnotation.setSelectionPrefix(annotationText.substring(0,
					annotationLength));
			textAnnotation.setSelectionSuffix(annotationText.substring(
					annotationText.length() - annotationLength,
					annotationText.length()));
			if (end < textLength)
				textAnnotation.setSelectionTail(text.substring(end + 1, end
						+ tailLength));
			else
				textAnnotation.setSelectionTail("");

			logger.trace(
					"[ textLength :: {} ][ headLength :: {} ][ tailLength :: {} ][ annotationLength :: {} ][ selectionHead :: {} ][ selectionPrefix :: {} ][ selectionSuffix :: {} ][ selectionTail :: {} ].",
					new Object[] { textLength, headLength, tailLength,
							annotationLength,
	...
	public void enhanceAndWriteTextAnnotationsForContentItem(
			ContentItem contentItem) {
		...
		try {
			...
			for (TextAnnotation textAnnotation : textAnnotations) {
				...
				graph.add(new TripleImpl(subjectURI, FISE_SELECTION_PREFIX_URI,
						new PlainLiteralImpl(textAnnotation
								.getSelectionPrefix())));
				graph.add(new TripleImpl(subjectURI, FISE_SELECTION_HEAD_URI,
						new PlainLiteralImpl(textAnnotation.getSelectionHead())));
				graph.add(new TripleImpl(subjectURI, FISE_SELECTION_TAIL_URI,
						new PlainLiteralImpl(textAnnotation.getSelectionTail())));
				graph.add(new TripleImpl(subjectURI, FISE_SELECTION_SUFFIX_URI,
						new PlainLiteralImpl(textAnnotation
								.getSelectionSuffix())));
			}
		} finally {
